...
If both pixels do not belong to it, then the evidence weight $\evidW_i$ goes to zero (\TODO{Fig. \ref{fig:mask_cases}c in supplementary material}).
...
both pixels belong to it (fuzzy AND operator at line 7 in Alg. \ref{alg:computing_affinities}), so by transitivity we conclude they should be in the same instance and their affinity $a_i$ should tend to one (\TODO{Fig. \ref{fig:mask_cases}a}); 
ii) only one pixel belongs to the $\mathcal{M}_{\coord{c}_i}$ mask and their affinity should tend to zero (\TODO{Fig. \ref{fig:mask_cases}b}). 
