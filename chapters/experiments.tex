...
\textbf{Baseline Model (SNB)} -- As a strong baseline, we re-implement the current state-of-the-art and train a model to predict affinities for a sparse neighborhood structure (Fig. \ref{fig:main_figure}a). We perform deep supervision by attaching three \emph{\sparseBr branches} (SNB) at different levels in the hierarchy of the UNet decoder and train the coarser feature maps to predict longer range affinities. Details about the used neighborhood structures can be found in Table \ref{tab:neighborhood_structures}.
...
% % UNet+ENB+AggrAff & WSDT+GASP-Avg & 0.248 & 0.323 \\
        ...
        % }
        \vspace{1em}
        \caption{Comparison experiments on our CREMI validation set. All models that were trained to predict \maskname masks (ENB) achieved better scores than the strong baseline SNB representing the current state-of-the-art method. The model that performed best was the one using the method proposed in Sec. \ref{sec:aggr_affs} to average overlapping masks (AggrAff).} \label{tab:val_results}
\end{table}
...
PNIUNet+SNB+EffAff \cite{lee2017superhuman} & \UPDATE{Z-Watershed + Agglo}  & 0.228 & \\
\UPDATE{GaspUNet}+SNB+EffAff \cite{bailoni2019generalized} & GaspAvg & 0.241 & $\checkmark$ \\
