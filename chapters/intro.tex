...
Our method achieves competitive scores on the challenging CREMI 2016 segmentation benchmark on neuron segmentation. In our set of validation experiments, we show how predicting encoded \maskname masks always improves training performances. Moreover, when predictions from overlapping masks are combined into edge weights of a graph that is subsequently partitioned, this yields a noise-robust algorithm, which gives priority to predictions sharing the highest consensus across predicted masks. 
This parameter-free algorithm, for the first time, performs better than other super-pixel based methods, which has always been the default choice on the challenging data from the CREMI competition challenge because of their robustness to noise.
